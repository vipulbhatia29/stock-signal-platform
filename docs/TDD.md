...
| `FeatureEngineering` | `backend/services/feature_engineering.py` | Pure vectorized functions for ML feature computation. 12 functions: momentum (3 windows), RSI, MACD histogram, SMA cross, BB position, volatility, Sharpe, forward log returns, ADX series, OBV slope series, MFI series. Gate indicators (ADX/OBV/MFI) used by confirmation-gate scoring v2. `build_feature_dataframe()` orchestrates all features for backfill + daily pipeline. |
